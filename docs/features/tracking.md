...
`_TestScoreChart` plots one point per calendar day (the averaged accuracy from `computeLast30DaysScores`), not one point per raw test attempt. X-axis labels use real MM/DD dates via `shortDateLabel`, reusing `_WeeklyChart`'s label `TextStyle` for visual consistency between the two charts. X-axis label density is thinned to ~6 visible labels max (`labelInterval = ceil(scores.length / 6)`) so dates stay legible over a full 30-day window; the off-screen `Semantics` summary still lists every date regardless of thinning. `_ScoreTable`'s date column is labeled "Date" (not "Test #") and renders the full `isoDateKey` (yyyy-MM-dd) per row; the column's `numeric` flag is `false` since it now holds a date string, not an index.

### Day-bucketing relies on local timestamps
`computeLast30DaysScores` groups raw `test_results` rows by `isoDateKey(tested_at)` (local calendar day, not UTC), averaging `accuracy` for all tests taken the same day. This assumes `tested_at` is persisted as a local (non-UTC) ISO timestamp by `DatabaseHelper.logTestResult` — if that ever changes to UTC, day boundaries would shift for users outside UTC.
