...
### Save-Confirmation Destination Preview and "Save and Add More" (#146, #147, #153)
- **Destination preview line.** The save-confirmation `AlertDialog` now shows "Will be saved to: Memorized" or "Will be saved to: Available" beneath the reference/text preview, reflecting the current `_saveAsMemorized` checkbox state at confirm time.
- **"Save and add more" checkbox.** A second `CheckboxListTile` below "Add directly to Memorized", bound to `_saveAndAddMore` (default `false`). When checked, the dialog's primary button label changes to "Save and add more" (same `add-verse-confirm-save-button` key); confirming calls `_resetFormForAnotherVerse()` instead of popping the screen — clears the reference/text controllers, resets translation to the settings default (with the existing ESV-unavailable→BSB fallback), resets `_saveAsMemorized` to `false`, clears error/warning state, and refocuses the reference field. `_saveAndAddMore` itself stays checked across saves (intentional, for rapid repeated entry). Unchecked (default) behavior is unchanged — saving pops the screen.
...
### Save Button Focus Restoration Fix (#146)
The save-confirmation `AlertDialog` (opened by tapping "Save Verse", not Search) was wrongly restoring focus to `_searchFocusNode` after closing — a stale copy-paste from the ESV/Bible consent dialog pattern, since those two dialogs are opened by the Search button and correctly own `_searchFocusNode`. Fixed by adding a dedicated `_saveFocusNode`, wired to the "Save Verse" button (key `add-verse-save-button`); the save-confirmation dialog now calls `_saveFocusNode.requestFocus()` after closing on both the Cancel and Save paths. The ESV/Bible consent dialogs are unchanged and still restore focus to `_searchFocusNode`.

### Reference-Resolution Call Deduplication (#147, #153)
Tapping Save immediately after editing the reference field previously fired two independent, concurrent DB-backed reference-resolution calls: one from `_normalizeReferenceOnBlur()` (triggered because tapping Save unfocuses the reference field) and one from `_saveVerse()` itself, both calling `_resolveReference` for the same raw input. Fixed with a `_resolveReferenceDeduped(rawReference)` helper that caches the in-flight `Future` for a given raw reference string via `_pendingResolutionFuture`/`_pendingResolutionInput` fields — a second caller requesting resolution for the same raw text while the first is still pending reuses that Future instead of starting a new DB call. Both `_normalizeReferenceOnBlur` and `_saveVerse` now call `_resolveReferenceDeduped` instead of `_resolveReference` directly.
...
| 2026-07-07 | Add Verse focus/dedup fixes (#146, #147, #153): added dedicated `_saveFocusNode` (wired to `add-verse-save-button`) so the save-confirmation dialog restores focus there instead of wrongly reusing `_searchFocusNode`; added `_resolveReferenceDeduped` to prevent `_normalizeReferenceOnBlur` and `_saveVerse` from firing duplicate concurrent reference-resolution DB calls when Save is tapped right after editing the reference field |
| 2026-07-07 | Save-confirmation dialog now previews destination ("Will be saved to: Memorized/Available") and adds a "Save and add more" checkbox that resets the form (`_resetFormForAnotherVerse`) instead of closing the screen on confirm, for rapid repeated entry |
