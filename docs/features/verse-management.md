...
On `.unresolvedBook`/`.invalidFormat`, save is blocked: the screen shows a field-level error (for screen-reader association with the reference input via `aria-describedby`-equivalent) plus an `InlineStatusBanner` with a link to the Book Name Variants settings screen. Editing the reference field after a successful normalization clears `_pendingNormalizedReference`, forcing re-normalization before the next Save tap can commit. Already-saved verses are never touched or backfilled by this feature — it only gates new saves.
