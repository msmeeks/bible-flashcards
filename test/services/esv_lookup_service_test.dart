  ...
  group('EsvLookupService.lookup', () {
    test('returns VerseLookupResult with translation ESV on success', () async {
      final service = EsvLookupService(
        client:
            _mockClient(200, '{"passages": ["For God so loved the world. "]}'),
        apiKey: 'test-key',
      );
    ...
    test('throws LookupException on 404', () async {
      final service =
          EsvLookupService(client: _mockClient(404, '{}'), apiKey: 'test-key');
      await expectLater(
          service.lookup('John 3:16'), throwsA(isA<LookupException>()));
      service.dispose();
    });

    test('throws LookupException on timeout', () async {
      final service =
          EsvLookupService(client: _timeoutClient(), apiKey: 'test-key');
      await expectLater(
    ...
    test('throws LookupException on network error', () async {
      final service =
          EsvLookupService(client: _errorClient(), apiKey: 'test-key');
      await expectLater(
          service.lookup('John 3:16'), throwsA(isA<LookupException>()));
      service.dispose();
    });
    ...
    test('throws LookupException on bad JSON', () async {
      final service = EsvLookupService(
          client: _mockClient(200, 'not json {'), apiKey: 'test-key');
      await expectLater(
          service.lookup('John 3:16'), throwsA(isA<LookupException>()));
      service.dispose();
    });
    ...
    test('throws LookupException when passages list is empty', () async {
      final service = EsvLookupService(
          client: _mockClient(200, '{"passages": []}'), apiKey: 'test-key');
      await expectLater(
          service.lookup('John 3:16'), throwsA(isA<LookupException>()));
      service.dispose();
    });
    ...
    test('throws LookupException for unknown book name', () async {
      final service =
          EsvLookupService(client: _mockClient(200, '{}'), apiKey: 'test-key');
      await expectLater(
          service.lookup('FakeBook 1:1'), throwsA(isA<LookupException>()));
      service.dispose();
    });

    test('isAvailable is false when API key is empty', () {
      final service =
          EsvLookupService(client: _mockClient(200, '{}'), apiKey: '');
    ...
    test(
        'isApiKeyConfigured reflects the compiled API key without constructing a client',
        () {
    ...
    test('lookup throws StateError when API key is empty', () async {
      final service =
          EsvLookupService(client: _mockClient(200, '{}'), apiKey: '');
      await expectLater(
          service.lookup('John 3:16'), throwsA(isA<StateError>()));
      service.dispose();
    });
