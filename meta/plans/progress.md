...
- **#143, #144**: Added widget tests for the two previously-uncovered branches: a transient `MicPermissionResult.denied` (announcement shown, settings dialog NOT opened — that dialog is reserved for `permanentlyDenied`) and `listen()` resolving `false` (unavailable announcement shown, listening state resets). New fakes: `_TransientlyDeniedSpeechService` and a `listenReturnsFalse` flag on the existing `_FakeSpeechService`.
- **#152**: Tapping "stop listening" no longer re-arms the 15s start-side wedge-detection timeout. Added a distinct `_postStopTimeoutDuration` (4s); `_startMicTimeout` now takes an optional `duration` override, and the explicit-stop path in `_onMicPressed` passes the shorter value, so an unresponsive plugin resolves the "Listening…" UI within ~4s of an explicit stop instead of up to 15s. The original 15s start-timeout behavior is unchanged.
- **#154**: Added a one-line comment next to `pauseFor: const Duration(seconds: 15)` in `speech_recognition_service.dart` explaining it's chosen to match the UI's wedge-detection timeout.
- **#157**: Replaced the screen's remaining default `Icons.*` usages (`close`, `check_rounded`, `close_rounded`, `check_circle_rounded`, `warning_amber_rounded`, `cancel_rounded`) with their Material Symbols Rounded equivalents, matching the rest of the screen's iconography.

TDD'd the post-stop timeout: added a test tapping stop on a wedged fake service (never calls `onStopped`), confirmed RED at 5s (still "Listening…" under the old shared 15s timer), then GREEN after introducing the separate 4s duration. The two branch-coverage tests were found already added (uncommitted) from a prior session's in-progress attempt; verified they pass and left them as-is. Full suite (520 tests) + `flutter analyze` pass (only pre-existing, unrelated deprecation infos remain). Updated `docs/features/test-modes.md` via the doc-writer agent.
